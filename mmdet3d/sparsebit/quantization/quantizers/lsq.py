...
from mmdet3d.sparsebit.quantization.quantizers import Quantizer as BaseQuantizer
from mmdet3d.sparsebit.quantization.quantizers import register_quantizer
...
class Quantizer(BaseQuantizer):
    ...
    def _forward(self, x, scale, zero_point):
        ...
        scale = gs_scaling().forward(scale, gs_ratio)
        # x_dq = STE.apply(x, scale, zero_point, self.qdesc, self.backend)
        x_dq = STE().forward(x, scale, zero_point, self.qdesc, self.backend)
        return x_dq
