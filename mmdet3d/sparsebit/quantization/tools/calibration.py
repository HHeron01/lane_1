...
from mmdet3d.sparsebit.quantization.modules import QuantOpr
from mmdet3d.sparsebit.quantization.quantizers.adaround import reconstruct_qlayer
...
class CalibrationRunner(object):
    ...
    def run_feature_calibration(self, node, asym=False):
        ...
        if (
            ...
            for inp_node in node.all_input_nodes:
                # print("inp_node:", inp_node, inp_node.target)
                ...
                for inp_tensor in inp_tensors:
                    if isinstance(inp_tensor, torch.Tensor):
                        # print("inp_tensor:", inp_tensor.shape, module, node)
