...
from mmdet3d.sparsebit.utils import update_config
from mmdet3d.sparsebit.quantization.modules import *
from mmdet3d.sparsebit.quantization.observers import Observer
from mmdet3d.sparsebit.quantization.quantizers import Quantizer
from mmdet3d.sparsebit.quantization.tools import QuantizationErrorProfiler
from mmdet3d.sparsebit.quantization.converters import simplify, fuse_operations
from mmdet3d.sparsebit.quantization.quant_tracer import QTracer
...
class QuantModel(nn.Module):
    ...
    def _convert2quantmodule(self):
        ...
        def _get_new_qmodule(target, org_module):
            # print("org_module:", target, org_module)
            is_matched = False
            # bev_lane_head
            # if target.split('.')[0] == 'bev_lane_head':
            # if target.split('.')[0] == 'img_view_transformer':
            if target.split('.')[0] != 'img_backbone':
                is_matched = True
            ...
            if is_matched:
                ...
                try:
                    ...
                    print("ERROR_module:", target, org_module, type(org_module))
            return new_module
        ...
        traced = self.model
        # print("traced.graph.nodes:", traced.graph.nodes)
        qnodes = []  # 用于避免重复遍历
        for n in traced.graph.nodes:
            # try:
            #     if n == 'backbone_maxpool_1':
            #         print("n:", n)
            # except:
            #     pass
            print("n:", n)
            if not isinstance(n, fx.Node) or n in qnodes:
                ...
                if org_module.__module__.startswith("sparsebit.quantization"):
                    qnodes.append(n)
                    continue
                # print("n.target:", n.target, org_module)
    ...
    def prepare_calibration(self):
        ...
        """
        from mmdet3d.sparsebit.quantization.tools.calibration import CalibrationRunner
    ...
    def set_quant(self, w_quant=False, a_quant=False):
        for n, m in self.model.named_modules():
            # print("m:", m, n)
            # if not isinstance(m, QuantOpr):
            #     print("m:", m, n)
            if isinstance(m, QuantOpr):
                # print("n, m:", n, m)
                m.set_quant(w_quant, a_quant)
            # if m == 'add_6' or 'add_5' or 'add_4' or 'add_3' or 'add_2' or 'add_1' or 'add':
            #     # m.set_quant(w_quant, a_quant)
            #     m.input_quantizer.set_fake_fused()
            # if m == 'backbone_maxpool':
            #     # print("m:", m)
            #     m.set_quant(w_quant, a_quant)
                # m.input_quantizer.set_fake_fused()
            # if m == ''
    ...
    def dump_mermaid(self, f):
        from mmdet3d.sparsebit.quantization.converters.utils.subgraph_matching_utils import (
            get_operators_type,
        )
        import mmdet3d.sparsebit.quantization.modules as M
