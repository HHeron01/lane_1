...
class BEVLane(Base3DDetector):
    def __init__(self, img_backbone, img_neck, img_view_transformer, img_bev_encoder_backbone,
        ...
        super(BEVLane, self).__init__(**kwargs)
        if img_backbone:
    ...
    def image_encoder(self, img):
        ...
        imgs = imgs.view(B * N, C, imH, imW)  # rader(N)
        x = self.img_backbone(imgs)  # 8，64， 40， 60
        if self.with_img_neck:
            x = self.img_neck(x)
            if type(x) in [list, tuple]:
                x = x[0]
        _, output_dim, ouput_H, output_W = x.shape
        x = x.view(B, N, output_dim, ouput_H, output_W)  #(8, 1, 64, 40, 64)  ---》（174， 64， 1， 1）, N: 雷达数量
        return x
    ...
    def forward_lane_train(self, img_feats, maps, **kwargs):
    ...
    def forward_train(self,
        ...
        img_feats = self.extract_feat(img=img_inputs, img_metas=img_metas, **kwargs)  # 18, 256, 480, 192
        losses = dict()
        losses_lane, out = self.forward_lane_train(img_feats, maps)
    ...
    def forward_test(self,
                      ...
                      maps=None,
                      **kwargs):
        return self.simple_test(img_metas, img_inputs, maps)
