...
class LoadMapsFromFiles(object):
    ...
    def __call__(self, results):
        ...
        cv2.imwrite('./maps.png', map_mask * 100)
...
class LoadMultiViewImageFromMultiSweepsFiles(object):
    ...
    def __init__(self,
                 ...
                 sensors=['CAM_FRONT', 'CAM_FRONT_RIGHT', 'CAM_FRONT_LEFT', 'CAM_BACK', 'CAM_BACK_LEFT',
                          'CAM_BACK_RIGHT'],
                 test_mode=True,
                 prob=1.0,
                 ):
...
@PIPELINES.register_module()
class LoadAnnotationsBevseg(object):
    ...
    def __call__(self, results):
        output = {}
        imgs, trans, rots, intrins, post_trans, post_rots, gt_mask, extrinsics, undists, mask_offset, \
        mask_haf, mask_vaf, gt_lanes, grid, drop_idx, mask_z, file_path, haf_masked = \
            results['imgs'], results['trans'], results['rots'], results['intrins'], results['post_trans'],\
            results['post_rots'], results['semantic_masks'], results['extrinsics'],\
            results['undists'], results['mask_offset'], results['mask_haf'], \
            results['mask_vaf'], results['gt_lanes'], results['grid'], results['drop_idx'], \
            results['mask_z'], results['file_path'], results['haf_masked']
        rotate_bda, scale_bda, flip_dx, flip_dy = self.sample_bda_augmentation(
        )
        # bda_rot = self.bev_transform(rotate_bda, scale_bda,
        #                                        flip_dx, flip_dy)

        bda_rot = self.bev_transform(0, 1, False, False)

        # output['img_inputs'] = (imgs, rots, trans, intrins, post_rots,
        #                          post_trans, bda_rot, extrinsics, undists, gt_lanes)

        output['img_inputs'] = (imgs, intrins, extrinsics, post_rots, post_trans, undists,
                                bda_rot, rots, trans, grid, drop_idx)

        # ks, imu2cs, post_rots, post_trans, undists
        output['maps'] = (gt_mask, mask_haf, mask_vaf, mask_offset, mask_z,haf_masked)
        output['gt_lanes'] = gt_lanes
        output['file_path'] = file_path

        return output

@PIPELINES.register_module()
class LoadAnnotationsBevAnchor(object):
    def __init__(self, bda_aug_conf, classes, is_train=True):
        self.bda_aug_conf = bda_aug_conf
        self.is_train = is_train
        self.classes = classes
    ...
    def __call__(self, results):
        output = {}
        imgs, trans, rots, intrins, post_trans, post_rots, extrinsics, undists, gt_lanes, grid, drop_idx, file_path, origin_lanes, gt_anchors = \
            results['imgs'], results['trans'], results['rots'], results['intrins'], results['post_trans'],\
            results['post_rots'], results['extrinsics'],\
            results['undists'], results['gt_lanes'], results['grid'], results['drop_idx'],\
            results['file_path'], results['origin_lanes'], results['gt_anchors']
        ...
        output['img_inputs'] = (imgs, intrins, extrinsics, post_rots, post_trans, undists,
                                bda_rot, rots, trans, grid, drop_idx) #gt_lanes)

        if gt_anchors is not None:
            output['targets'] = gt_anchors
        else:
            output['targets'] = gt_lanes
        output['file_path'] = file_path
        output['origin_lanes'] = origin_lanes

        return output

@PIPELINES.register_module()
class LoadAnnotationsSeg2D(object):
    ...
    def __call__(self, results):
        output = {}
        imgs, trans, rots, intrins, post_trans, post_rots, extrinsics, undists,mask_bev, mask_2d \
        , gt_lanes_3d, gt_lanes_2d, grid, drop_idx, file_path = \
            results['imgs'], results['trans'], results['rots'], results['intrins'], results['post_trans'],\
            results['post_rots'], results['extrinsics'],\
            results['undists'], results['mask_bev'], results['mask_2d'], \
            results['gt_lanes_3d'], results['gt_lanes_2d'],results['grid'], results['drop_idx'], \
            results['file_path']
        rotate_bda, scale_bda, flip_dx, flip_dy = self.sample_bda_augmentation(
        )
        # bda_rot = self.bev_transform(rotate_bda, scale_bda,
        ...
        # ks, imu2cs, post_rots, post_trans, undists
        output['maps_2d'] = mask_2d
        output['maps_bev'] = mask_bev
        output['gt_lanes_3d'] = gt_lanes_3d
        output['gt_lanes_2d'] = gt_lanes_2d
        output['file_path'] = file_path
        return output

# 得到所有参数之后，有一些参数涉及BEV的需要做转换，在这个函数中添加转换，（也就是对原始数据套了一层），然后返回转换后的数据
@PIPELINES.register_module()
class LoadAnnotationsSeg(object): 
    # 注册LoadAnnotationsSeg类为数据处理管道的装饰器

    # 初始化方法，接受三个参数：bda_aug_conf用于BEV数据增强的配置，classes表示类别信息，is_train表示是否处于训练模式
    def __init__(self, bda_aug_conf, classes, is_train=True):
        self.bda_aug_conf = bda_aug_conf  # 保存BEV数据增强的配置
        self.is_train = is_train  # 记录是否处于训练模式
        self.classes = classes  # 保存类别信息

    # 生成BEV数据增强的参数，根据训练模式随机生成旋转角度、缩放比例、以及是否进行镜像翻转的参数
    def sample_bda_augmentation(self):
        if self.is_train:  # 如果处于训练模式
            rotate_bda = np.random.uniform(*self.bda_aug_conf['rot_lim'])  # 随机生成旋转角度
            scale_bda = np.random.uniform(*self.bda_aug_conf['scale_lim'])  # 随机生成缩放比例
            flip_dx = np.random.uniform() < self.bda_aug_conf['flip_dx_ratio']  # 根据概率决定是否进行水平镜像翻转
            flip_dy = np.random.uniform() < self.bda_aug_conf['flip_dy_ratio']  # 根据概率决定是否进行垂直镜像翻转
        else:  # 如果不处于训练模式
            rotate_bda = 0  # 旋转角度设为0
            scale_bda = 1.0  # 缩放比例设为1.0
            flip_dx = False  # 不进行水平镜像翻转
            flip_dy = False  # 不进行垂直镜像翻转
        return rotate_bda, scale_bda, flip_dx, flip_dy  # 返回生成的BEV数据增强参数

    # 对BEV数据进行变换，根据旋转角度、缩放比例、水平镜像和垂直镜像进行相应的变换
    def bev_transform(self, rotate_angle, scale_ratio, flip_dx, flip_dy):
        rotate_angle = torch.tensor(rotate_angle / 180 * np.pi)  # 将旋转角度转换为弧度
        rot_sin = torch.sin(rotate_angle)  # 计算旋转角度的正弦值
        rot_cos = torch.cos(rotate_angle)  # 计算旋转角度的余弦值
        rot_mat = torch.Tensor([[rot_cos, -rot_sin, 0],  # 构建旋转矩阵
                                [rot_sin, rot_cos, 0],
                                [0, 0, 1]])
        scale_mat = torch.Tensor([[scale_ratio, 0, 0],  # 构建缩放矩阵
                                  [0, scale_ratio, 0],
                                  [0, 0, scale_ratio]])
        flip_mat = torch.Tensor([[1, 0, 0], [0, 1, 0], [0, 0, 1]])  # 构建单位矩阵
        if flip_dx:  # 如果需要进行水平镜像翻转
            flip_mat = flip_mat @ torch.Tensor([[-1, 0, 0], [0, 1, 0], [0, 0, 1]])  # 乘以水平翻转矩阵
        if flip_dy:  # 如果需要进行垂直镜像翻转
            flip_mat = flip_mat @ torch.Tensor([[1, 0, 0], [0, -1, 0], [0, 0, 1]])  # 乘以垂直翻转矩阵
        rot_mat = flip_mat @ (scale_mat @ rot_mat)  # 先进行缩放、再进行旋转、最后进行翻转
        return rot_mat  # 返回变换矩阵

    def __call__(self, results):
        output = {}  # 创建一个字典用于保存处理结果

        # 从输入的`results`中获取各个数据部分
        # imgs, trans, rots, extrinsics, intrins, undists, post_trans, post_rots, mask_bev, mask_2d, \
        # gt_lanes_3d, gt_lanes_2d, grid, drop_idx, file_path = \
        # 图像数据
        imgs = results['imgs']
        # 相机位移数据
        ...
        # 内参矩阵数据
        intrins = results['intrins']
        # 畸变参数数据
        undists = results['undists']
        # 后处理位移数据
        post_trans = results['post_trans']
        # 后处理旋转数据
        post_rots = results['post_rots']
        # BEV掩码数据
        mask_bev = results['mask_bev']
        # 2D掩码数据
        mask_2d = results['mask_2d']
        # 3D车道线数据
        gt_lanes_3d = results['gt_lanes_3d']
        # 2D车道线数据
        gt_lanes_2d = results['gt_lanes_2d']
        # 网格数据
        grid = results['grid']
        # 要删除的数据索引
        ...
        # 调用`sample_bda_augmentation`方法生成BEV数据增强参数，但这里注释掉了
        # rotate_bda, scale_bda, flip_dx, flip_dy = self.sample_bda_augmentation()
        
        # 调用`bev_transform`方法进行BEV数据的变换，参数为0, 1, False, False
        bda_rot = self.bev_transform(0, 1, False, False)

        # 将各个数据部分组合成`img_inputs`键对应的元组，并保存到`output`字典中
        output['img_inputs'] = (imgs, intrins, extrinsics, post_rots, post_trans, undists,
                                bda_rot, rots, trans, grid, drop_idx)
        # 将`mask_bev`保存到`maps_bev`键中
        output['maps_bev']  =  mask_bev
        # 将`mask_2d`保存到`maps_2d`键中
        output['maps_2d']   = mask_2d
        # 将`gt_lanes_3d`保存到`gt_lanes_3d`键中
        output['gt_lanes_3d']  = gt_lanes_3d
        # 将`gt_lanes_2d`保存到`gt_lanes_2d`键中
        output['gt_lanes_2d'] = gt_lanes_2d
        # 将`file_path`保存到`file_path`键中
        output['file_path'] = file_path
        return output  # 返回保存处理结果的字典
