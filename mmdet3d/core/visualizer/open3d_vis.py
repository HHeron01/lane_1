...
class Visualizer(object):
    """使用Open3d实现的在线可视化工具。
    参数：
    points (numpy.array, shape=[N, 3+C]): 要可视化的点云数据。点云数据的坐标模式为Coord3DMode.DEPTH（请参考core.structures.coord_3d_mode）。
    bbox3d (numpy.array, shape=[M, 7], 可选): 要可视化的3D边界框数据（x、y、z、x尺寸、y尺寸、z尺寸、偏航角）。3D边界框数据的模式为Box3DMode.DEPTH，中心点为重心（请参考core.structures.box_3d_mode）。默认值: None。
    save_path (str, 可选): 可视化结果保存的路径。默认值: None。
    points_size (int, 可选): 在可视化中点的大小。默认值: 2。
    point_color (tuple[float], 可选): 点的颜色。默认值: (0.5, 0.5, 0.5)。
    bbox_color (tuple[float], 可选): 边界框的颜色。默认值: (0, 1, 0)。
    points_in_box_color (tuple[float], 可选): 边界框内的点的颜色。默认值: (1, 0, 0)。
    rot_axis (int, 可选): 边界框的旋转轴。默认值: 2。
    center_mode (bool, 可选): 指示边界框的中心是底部中心还是重心。可用的模式为['lidar_bottom', 'camera_bottom']。默认值: 'lidar_bottom'。
    mode (str, 可选): 输入点云的类型，可用的模式为['xyz', 'xyzrgb']。默认值: 'xyz'。
    """
    ...
    def add_bboxes(self, bbox3d, bbox_color=None, points_in_box_color=None):
        """将边界框添加到可视化工具中。
        参数：
        bbox3d (numpy.array, shape=[M, 7]): 要可视化的3D边界框数据（x、y、z、x尺寸、y尺寸、z尺寸、偏航角）。3D边界框数据的模式为Box3DMode.DEPTH，中心点为重心（请参考core.structures.box_3d_mode）。
        bbox_color (tuple[float]): 边界框的颜色。默认值: None。
        points_in_box_color (tuple[float]): 边界框内的点的颜色。默认值: None。
        """
    ...
    def add_seg_mask(self, seg_mask_colors):
        """通过每个点的颜色化将分割掩码添加到可视化工具中。

        参数：
        seg_mask_colors (numpy.array, shape=[N, 6]):
        分割掩码，其中前3个维度是点的坐标，后3个维度是转换后的颜色。
        """
