...
class BboxOverlapsNearest3D(object):
    ...
    最近邻3D IoU计算器。
    注意：
        此IoU计算器首先在鸟瞰图（BEV）中找到最近的2D框，然后使用:meth:`bbox_overlaps`计算2D IoU。
    参数：
        coordinate (str)：坐标系统，可以是 'camera'、'lidar' 或 'depth'。
    """
    ...
    def __call__(self, bboxes1, bboxes2, mode='iou', is_aligned=False):
        """计算最近邻3D IoU。
        注意：
        如果 ``is_aligned`` 是 ``False``，则计算bboxes1和bboxes2之间的IoU，否则计算bboxes1和bboxes2之间每对对齐框的IoU。
        参数：
            bboxes1 (torch.Tensor): 形状为 (N, 7+N) 的张量，表示框的坐标和大小 [x, y, z, x_size, y_size, z_size, ry, v]。
            bboxes2 (torch.Tensor): 形状为 (M, 7+N) 的张量，表示框的坐标和大小 [x, y, z, x_size, y_size, z_size, ry, v]。
            mode (str): 计算模式，可以是 "iou"（交并比）或 "iof"（前景交并比）。
            is_aligned (bool): 计算是否为对齐计算。
        返回值：
            torch.Tensor: 如果 ``is_aligned`` 为 ``True``，返回bboxes1和bboxes2之间的IoU，形状为 (M, N)。如果 ``is_aligned`` 为 ``False``，返回形状为 M 的张量。
        """
