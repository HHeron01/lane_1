...
class BaseInstance3DBoxes(object):
    ...
    三维框的基类。
    注意:
        该框底部居中，即框的原点相对于框是（0.5，0.5，0）。
    参数:
        tensor (torch.Tensor | np.ndarray | list): 一个 N x box_dim 的矩阵。
        box_dim (int): 框的维度数。
            每一行是（x，y，z，x_size，y_size，z_size，yaw）。
            默认为7。
        with_yaw (bool): 框是否具有偏航角旋转。
            如果为False，则将yaw的值设置为0，如最小最大框那样。
            默认为True。
        origin (tuple[float], optional): 框原点的相对位置。
            默认为（0.5, 0.5, 0）。这将指导框转换为（0.5，0.5，0）模式。
    属性:
        tensor (torch.Tensor): N x box_dim 的浮点矩阵。
        box_dim (int): 框的维度数，表示框的维度。
            每一行是（x，y，z，x_size，y_size，z_size，yaw, ...）。
        with_yaw (bool): 如果为True，则将yaw的值设置为0，如最小最大框那样。
    """
