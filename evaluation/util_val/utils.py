...
def define_args():
    ...
    parser.add_argument('--new_match', type=str2bool, nargs='?', const=True, default=False, help='Allow multiple anchors to match the same GT during 3D loader loading')
...
class Visualizer:
    ...
    def save_result_new(self, dataset, train_or_val, epoch, idx, images, gt, pred, pred_cam_pitch, pred_cam_height,
        ...
        for i in range(idx.shape[0]):
            ...
            P_gt = np.matmul(H_crop, H_g2im) # H_crop 是图像级别的resize
...
def homograpthy_g2im_extrinsic(E, K):
    ...
    H_g2c = E_inv[:, [0,1,3]] # groud2camera
    H_g2im = np.matmul(K, H_g2c)
    return H_g2im # 3*3 这个之后绕z的旋转ground2camera
...
#         # pack prediction loader to nms library format
...
def unit_update_projection_for_data_aug(args, aug_mats, _M_inv, _S_im_inv=None, _S_im=None):
    """
        Unit function to update transformation matrix when loader augmentation have been applied, and the image augmentation matrix are provided
